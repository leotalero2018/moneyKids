...
- **Kid device sessions:** each kid has a join code / QR. A Cloud Function validates the code and mints a Firebase **custom auth token** for a **stable Auth UID** (`kid_{familyId}_{kidId}` — family-namespaced, since kid IDs are only unique within a family while Auth UIDs are global) with `familyId`, `kidId`, `role: kid` claims. Codes expire and are revocable from parent settings. Revoking a kid's access calls `revokeRefreshTokens()` on that UID (not just the code): the device cannot refresh again, and any already-issued ID token expires within its normal ≤1-hour lifetime — that is the documented maximum residual access window.
...
- `families/{familyId}` — name, language preference, currency (ISO 4217; **immutable after creation, rule-enforced** — a currency change would make the ledger ambiguous, so it requires creating a new family), createdBy, deduction rules `[{name (ES/EN), basisPoints, destination: savings | withheld}]`. Rules are validated on write: each `basisPoints` is a non-negative integer, and the sum across rules is ≤ 10000 (100%) — net can never go negative. Rules are **snapshotted at approval**: the approval function computes the breakdown from the rules current at that moment, which is exactly what the parent sees on the review screen before confirming; changing family rules never affects already-approved invoices. The per-kid **deductions-enabled flag is honored by the approval function, not just the UI**: for a disabled kid it applies no rules at all — `deductions` is frozen empty and `netAmount == approvedAmount`, with the full gross credited to spendable.
- `families/{familyId}/members/{userId}` — role (parent), display name. **A family can never exist without its founder as a member:** creation is rule-enforced to require the founder's own member doc in the same batch (`existsAfter(.../members/{uid})`), since a family whose only parent is missing from `members` is permanently unreachable — every other rule authorizes through membership, so nobody could read, write, or repair it.
...
- `families/{familyId}/invoices/{invoiceId}` — kidId, optional activityId, description, photo refs, status, and an immutable money/audit record: `requestedAmount` (the kid's current ask — initial value immutable in events, revisable on resend per the transition matrix), optional `counterOffer {amount, note, parentId, at}`, `approvedAmount` (gross, set only at approval), `deductions [{name, basisPoints, amount, destination}]` and `netAmount` (both computed at approval from the family's rules, frozen on the invoice), and an append-only `events` **subcollection** (create-only in rules — Firestore rules cannot enforce append-only arrays) where every transition records `{from, to, actorUid, at, note, requestedAmount?}` — `requestedAmount` is required on every `→ sent` event (rule-enforced) and captures the amount as of that send, so the full negotiation history survives later revisions. Events are **sequence-bound to the invoice**: the invoice carries an `eventCount` (0 at draft creation, +1 per transition) and event N lives at the deterministic ID `e{N}`, so a transition and its event must be written in the same batch — the invoice rule requires `existsAfter(.../events/e{newEventCount})` and the event rule validates `from` against the pre-batch status. Event-less transitions and fabricated standalone events are both structurally impossible. (Verified against the emulator: computed path segments are valid rules interpolation. Note that `existsAfter` on a missing document surfaces as a rules *evaluation error* rather than a clean `false` — it still fails closed, but negative tests must also assert the positive path to prove they deny for the right reason.)
...
**Photos:** Firebase Storage under `families/{familyId}/kids/{kidId}/invoices/{invoiceId}/`. **Reads:** parents may read any path in their family; kid sessions may read only paths whose `{kidId}` segment matches their `kidId` claim — siblings cannot see each other's photos. **Writes/deletes:** only the matching kid session (or a family parent), only while the linked invoice is editable per the matrix above (kid-editable states: `draft`, `returned`, `countered`), with limits of 5 MB per file and `image/*` content types. **The path's `{kidId}` segment must match the linked invoice's own `kidId`** — checking only the invoice's status would let a kid write under their own prefix using a *sibling's* editable invoice ID, so the Storage rule asserts both `status` and ownership from the same `firestore.get`. **Photo count is capped at 8 per invoice**, enforced on `photoPaths` in the Firestore invoice rules (where the array lives; Storage rules cannot count objects) — the per-file size limit alone permits unbounded uploads. The client reserves the Firestore auto-ID by creating the `draft` invoice doc before uploading.
...
- **Callable functions have their own authorization contract** — the Admin SDK bypasses Firestore rules, so rules protect nothing inside a function. Every callable (approval, payout, join-code minting, kid revocation) must independently verify, from its own auth context: the caller is an authenticated member of the target family with the required role, and every target ID (invoice, kid, activity) belongs to that same family. Required role is parent for approval, payout, join-code minting, and revocation — with one exception: **counter-offer acceptance** may be called by the owning kid (matching `kidId` claim), and it approves strictly at the parent's recorded `counterOffer.amount`, which the parent pre-authorized when countering. Cross-family and wrong-role calls are rejected. Emulator tests must cover both rejections for each function, and "cross-family" means a **real member of another family targeting this one** — a test using an unaffiliated stranger UID proves only the membership check, not family scoping.
- **Callable payloads are untrusted input and are validated before use.** Authorization is not validation: `familyId`, `kidId`, `invoiceId`, and `requestId` are interpolated into Firestore document paths, so an ID containing `/` re-points the path to a different document, and `requestId` additionally becomes part of a ledger document ID. Every callable validates, from shared validators, that IDs are non-empty bounded strings with no `/` or path-traversal segments, that `requestId` matches an opaque bounded format, that free-text (`note`) is a bounded string, and that join codes match the exact code alphabet and length. Malformed-input tests are required alongside the authorization tests.
...
- **Firebase emulator** tests for security rules and the approval/payout Cloud Functions (safety-critical: kids must never be able to credit themselves; client writes to ledger/balances must fail; unconstrained kid queries must fail; duplicate approval must fail; payout over balance must fail; a parent of another family must fail on every callable; malformed IDs must fail; a kid must not attach photos to a sibling's invoice; family creation without the founder member doc must fail).
- **Typechecking is part of verification:** Vitest transpiles without typechecking and esbuild strips types, so `strict: true` is only actually enforced by a `tsc --noEmit` pass per workspace, run alongside the tests.
